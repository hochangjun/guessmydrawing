...
import { ReactTogether, useStateTogether, useMyId, useAllNicknames, useConnectedUsers, useFunctionTogether, useChat } from 'react-together';
...
interface GameState {
  ...
  wagerAmount: number;
  lobbyOwner: string;
  guessedCorrectly: string[]; // user IDs who guessed correctly this round
}
...
// MultiSynq Configuration - You'll need to replace these with your actual credentials
const MULTISYNQ_CONFIG = {
  appId: 'io.multisynq.guessmydrawing',
  apiKey: 'your-multisynq-api-key-here' // Replace with actual API key
};
...
const GuessMyDrawingGame: React.FC = () => {
  ...
  useEffect(() => {
    if (myId && !players[myId]) {
      ...
      // Set as lobby owner if first player
  ...
  const startGame = () => {
    ...
    const readyPlayers = Object.values(players).filter(p => p.hasPaid && p.isReady);
  ...
  const nextRound = () => {
    ...
    const readyPlayers = Object.values(players).filter(p => p.hasPaid && p.isReady);
    const currentDrawerIndex = readyPlayers.findIndex(p => p.id === gameState.currentDrawer);
    ...
    if (isCorrect && !gameState.guessedCorrectly.includes(myId)) {
      ...
      setGameState(prev => ({
        ...
        scores: {
          ...
          [prev.currentDrawer!]: (prev.scores[prev.currentDrawer!] || 0) + 20 // Drawer bonus
        },
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {gameState.lobbyOwner === myId && (
                  <button
                    onClick={startGame}
                    disabled={Object.values(players).filter(p => p.hasPaid).length < 2}
